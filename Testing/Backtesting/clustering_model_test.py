...
### Constants --> need to be kept global for backtesting purposes?
...
### for caching ###
cache_fname = "historical/" + f"{stock_name}_hist.csv"
...
def main():
    ...
    if not os.path.exists(cache_fname): #gotta cache hard 
        hist.to_csv(cache_fname) #saves the file if doesn't exist --> the DF is directly converted into CSV that we can read off, no formatting
...
def backtest_cache_ret(): #helper function to cache prev results into CSV file if doesn't exist, and retrieve it for testing purposes, so dun hit API again
    ## failsafe for not caching
    if not os.path.exists(cache_fname): 
        stock = yf.Ticker(stock_name)  # this goes in main()
        hist = stock.history(period = data_period, interval = resolution) # historical price data
        hist.to_csv(cache_fname) #saves the file if doesn't exist --> the DF is directly converted into CSV that we can read off, no formatting
    
    pd_data = pd.read_csv(cache_fname)
